"""
CUDA_VISIBLE_DEVICES=0 python compare_runs.py --dataset CF --s2only
"""
...
def main(n_gpus, dataset, batch_size, top_n, s1only):
    ...
    '''
    print(f"{agreement.sum() / agreement.shape[0]*100:.2f},{correct_agreement.sum() / correct_1.sum()*100:.2f},{map_agreement.mean()*100:.2f}")
