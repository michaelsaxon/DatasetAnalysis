...
def main(dataset, s1only, s2only, n_clusters, n_components, lastdense):
    ...
    print(len(sentences_correct_order))
    ...
    with open(f"{intermed_comp_dir}/tsne_{n_clusters}_l2.csv","w") as f:
        f.writelines(["sid,label,clid,x,y,pecol2,txt\n"] + [f'{i},{labs[i]},{embs_cll[i]},{tsne_coords[i,0]},{tsne_coords[i,1]},{clusters_L2[embs_cll[i]]},"{sentences_correct_order[i].replace(",","")}"\n' for i in range(labs.shape[0])])
